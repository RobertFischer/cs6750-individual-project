$.when($.ready).then(function() {
	...
	var episode1 = new Audio("./part4.mp3");
	...
	var episode2 = new Audio("./tmw.mp3");
	...
	function doRun() {
		...
		episode1.play();
		$(episode1).one("ended", () => {
			var utter = new SpeechSynthesisUtterance("Next up: the podcast \"The Magician's Wife\". Episode: \"The Baker Street Bank Robbery\"")
			utter.lang = "en-US";
			utter.volume = 0.5;
			utter.pitch = 0;
			utter.voice = voice;
			utter.addEventListener("end", function() {
				_.delay(function() { 
					episode2.play();
					for(let i = 0 ; i < 100; i++) {
						_.delay(function() { episode2.volume = 0.5 - i * (0.5/100) }, 10 * 1000 + 100 * i);
					}
					_.delay(function() { 
						episode2.pause(); 
						tryRun();
